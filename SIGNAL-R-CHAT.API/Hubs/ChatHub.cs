...
namespace SIGNAL_R_CHAT.API.Hubs
{
    ...
    public class ChatHub : Hub
    {
        ...
        public async Task SendToRoom(string workGroupName, string text)
        {
            try
            {
                var person = _context.Persons.Where(u => u.UserName == IdentityName).FirstOrDefault();
        ...
        public async Task Join(string workGroupName)
        {
            try
            {
                var person = _Connections.Where(u => u.UserName == IdentityName).FirstOrDefault();
        ...
        public async Task CreateRoom(string workGroupName)
        {
            try
            {
                ...
                else
                {
                    ...
                    var user = _context.Persons.Where(u => u.UserName == IdentityName).FirstOrDefault();
        ...
        public override Task OnDisconnectedAsync(Exception exception)
        {
            try
            {
                var person = _Connections.Where(u => u.UserName == IdentityName).First();
                ...
                // Remove mapping
                _ConnectionsMap.Remove(person.UserName);
            }
            catch (Exception ex)
            {
